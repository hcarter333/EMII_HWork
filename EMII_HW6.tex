...
\\
\\
$\vec{\nabla} \times \vec{v}\phi = \phi \vec{\nabla}\times\vec{v} + \vec{nabla}\phi\times\vec{v}$
\\
\\
The first term is zero since $\vec{v}$ is constant.  Remembering that $\nabla\phi = \vec{E}$, we're left with 
\\
\\
$\vec{B} = \vec{E} \times \vec{v}$
\\
\\
\PRLsep
\\
...
\\
\\
First, substitute in the detailed expressions
\\
\\
